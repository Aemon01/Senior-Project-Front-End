...
type PopupProps<T extends HTMLElement = HTMLElement> = {
    overlayColor?: string;
    placement?:
    | "Centered"
    | "Top left"
    ...
    top?: number;
    bottom?: number;
    relativeLayerRef?: RefObject<T | null>;
};

function PortalPopup<T extends HTMLElement = HTMLElement>({
    children,
    overlayColor,
    placement = "Centered",
    onOutsideClick,
    ...
    const onOverlayClick = useCallback(
        (e: React.MouseEvent<HTMLDivElement>) => {
            if (onOutsideClick && (e.target as HTMLElement).classList.contains(styles.portalPopupOverlay)) {
                onOutsideClick();
            }
            e.stopPropagation();
        },
